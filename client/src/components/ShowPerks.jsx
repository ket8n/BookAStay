...
import IconPool from "../assets/icons/IconPool";
import IconSnow from "../assets/icons/IconSnow";
import IconCeilingFan from "../assets/icons/IconCeilingFan";

const ShowPerks = ({ perks }) => {
  const show = (str) => {
    let className = "flex gap-1";
    if (perks.find((it) => it === str) === undefined) {
  ...
  return (
    <div className="mb-4 grid gap-2 w-8/12 grid-cols-[2fr_1fr]">
      ...
      <div className={show("ac")}>
        <IconSnow />
        Air conditioning
      </div>
      <div className={show("fan")}>
        <IconCeilingFan />
        Ceiling Fan
      </div>
    </div>
